...
function server.login_handler(uid, secret, id)
	...
	-- trash subid (no used)
	skynet.call(agent, "lua", "login", uid, internal_id, secret, id)
	users[uid] = u
...
function server.agent_handler(username, fd)
	print("~~~~~~~~~~~~`", username, fd)
	local u = username_map[username]
	--skynet.rawcall(u.agent, "beat", "1241241")
	skynet.call(u.agent, "lua", "heartbeat", fd)
end
